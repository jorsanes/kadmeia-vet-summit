...
export default function PostDetail() {
  ...
  const data = getPostBySlug(lang as any, slug);
  if (!data) return <div className="container py-20">Post no encontrado.</div>;

  const { meta, mod } = data;
  const MDX = mod.default;

  const title = `${meta.title} | KADMEIA`;
  const desc = meta.excerpt ?? 'Artículo de KADMEIA';
